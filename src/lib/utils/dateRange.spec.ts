...
describe('getMonthRange', () => {
	it('通常の月（2025-10）の範囲を正しく返す', () => {
		...
		expect(from.toISOString()).toBe('2025-09-30T15:00:00.000Z');
		expect(toExclusive.toISOString()).toBe('2025-10-31T15:00:00.000Z');
	});

	it('年初の月（2025-01）の範囲を正しく返す', () => {
		const { from, toExclusive } = getMonthRange('2025-01');

		expect(from.toISOString()).toBe('2024-12-31T15:00:00.000Z');
		expect(toExclusive.toISOString()).toBe('2025-01-31T15:00:00.000Z');
	});

	it('年末の月（2025-12）の範囲を正しく返し、翌年1月初にまたぐ', () => {
		const { from, toExclusive } = getMonthRange('2025-12');

		expect(from.toISOString()).toBe('2025-11-30T15:00:00.000Z');
		expect(toExclusive.toISOString()).toBe('2025-12-31T15:00:00.000Z');
	});

	it('うるう年の2月（2024-02）は29日まで、toExclusiveは3月初', () => {
		const { from, toExclusive } = getMonthRange('2024-02');

		expect(from.toISOString()).toBe('2024-01-31T15:00:00.000Z');
		expect(toExclusive.toISOString()).toBe('2024-02-29T15:00:00.000Z');
	});

	it('平年の2月（2025-02）は28日まで、toExclusiveは3月初', () => {
		const { from, toExclusive } = getMonthRange('2025-02');

		expect(from.toISOString()).toBe('2025-01-31T15:00:00.000Z');
		expect(toExclusive.toISOString()).toBe('2025-02-28T15:00:00.000Z');
	});
