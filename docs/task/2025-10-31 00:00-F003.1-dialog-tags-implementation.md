...
- [ ] Props に `tags` と `tagSuggestions` が追加されている
- [ ] TagInput コンポーネントが表示される
- [ ] 既存のタグが初期表示される
- [ ] タグの追加・削除ができる
- [ ] オートコンプリートが機能する
- [ ] フォーム送信時に tags が含まれる
- [ ] バリデーションが機能する（最大20個、100文字以内）
- [ ] UT-2.1 のすべてのテストが合格する
- [ ] 型チェックが通る
...
- [ ] WorkLogList が tags を含めて詳細ダイアログに渡す
- [ ] +page.svelte が tagSuggestions を編集モーダルに渡す
- [ ] +page.server.ts が tagSuggestions をロードしている（確認）
- [ ] UT-3.1 のテストが合格する
- [ ] 型チェックが通る
